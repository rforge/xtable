...
\author{Jonathan Swinton and others\\<jonathan@swintons.net>}
\Plainauthor{Jonathan Swinton, and others} %% comma-separated
\Plaintitle{The xtable Gallery} %% without formatting
\Shorttitle{\pkg{xtable} Gallery} %% a short title (if necessary)
...
\Abstract{
  This document gives a gallery of tables which can be made by using
  the \texttt{xtable} package to create \LaTeX\ output.  It doubles as
  a regression check for the package.
}

\Keywords{Reproducible research, \LaTeX}
\Address{Jonathan Swinton\\E-mail: \email{jonathan@swintons.net}}
...
\vspace{12pt}\textbf{\itshape An anova object}

<<results='asis'>>=
xtable(anova(fm2))
@
\vspace{12pt}\textbf{\itshape Another anova object}
<<>>=
...
\vspace{12pt}\textbf{\itshape An anova object}
<<results='asis'>>=
xtable(anova(npk.aov))
@

\vspace{12pt}\textbf{\itshape Another anova object}
<<results='asis'>>=
xtable(summary(npk.aov))
@
...
useful for formatting tables in a sensible way.

<<preliminary>>=
dat <- mtcars[1:3, 1:6]
@ %def

Consider the output produced by the default formatting.
...
In a \LaTeX\ document this appears as follows.

<<currentresult, echo = FALSE, results = 'asis'>>=
x
@ %def

Now change the default alignment, digits and display using helper functions
\texttt{xalign}, \texttt{xdigits}, and \texttt{xdisplay}.
...
This produces a better format as shown below.

<<proposedresult, echo = FALSE, results = 'asis'>>=
x
@ %def
...
<<>>=
wanttex <- xtable(data.frame(label =
...
\vspace{12pt}
\textbf{\itshape Single string and column lines}
<<>>=
...
tli.table
@
\vspace{12pt}\textbf{\itshape Fixed width columns}
<<>>=
...
\vspace{12pt} If you want a vertical line on the left, you need to
change the \code{align} attribute.
<<>>=
...
\vspace{12pt} Revert the alignment to what is was before.
<<>>=
align(tli.table) <- "|rr|lp{3cm}l|r|"
@
...
The \texttt{rotate.rownames } and \texttt{rotate.colnames} arguments can be
...
\vspace{12pt}\textbf{\itshape Line locations}

Use the \texttt{hline.after} argument to specify the position of the
horizontal lines.
...
\vspace{12pt}\textbf{\itshape Line styles}

The \LaTeX package \pkg{ booktabs} can be used to specify different
line style tags for top, middle, and bottom lines.  Specifying
\code{ booktabs = TRUE} will lead to separate tags being generated
for the three line types.

Insert \verb|\usepackage{booktabs}| in your \LaTeX preamble and define
the \texttt{toprule}, \texttt{midrule}, and \texttt{bottomrule}
tags to specify the line styles. By default, when no value is given
for \texttt{hline.after}, a \texttt{toprule} will be drawn above the
...
<<results='asis'>>=
print(xtable(anova(glm.D93)), size = "small")
@
...
x.big <- xtable(x, label = 'tabbig',
                caption = 'Example of \\code{longtable} spanning several pages')
@
<<results='asis'>>=
...
%%
%
%\vspace{12pt}\textbf{\itshape Long tables with the header on each page}
%
%The \texttt{add.to.row} argument can be used to display the header
...
<<results='asis'>>=
print(x.small, floating = TRUE, floating.environment = 'sidewaystable')
@
\clearpage

\subsection{Rescaled tables}
Specify a \texttt{scalebox} value to rescale the table.
...
The \texttt{tabularx} tabular environment provides more alignment options,
and has a \texttt{width} argument to specify the table width.
...
By default the \texttt{print} method will print the LaTeX or HTML to standard
output and also return the character strings invisibly.  The printing to
standard output can be suppressed by specifying \texttt{print.results = FALSE}.
...
\texttt{toLatex} method.  This function returns an object of class
\texttt{"Latex"}.
...
Most of the examples in this gallery are taken from the \texttt{xtable}
documentation.
