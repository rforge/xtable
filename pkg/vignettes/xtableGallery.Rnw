...
\Abstract{This document gives a gallery of tables which can be made by using the
  \texttt{xtable} package to create \LaTeX\ output. It doubles as a regression
  check for the package.}
\Keywords{Reproducible research, \LaTeX}
\Shorttitle{\pkg{xtable} Gallery} %% JSS header
\Address{Jonathan Swinton\\E-mail: \email{jonathan@swintons.net}} %% JSS last pg
...
\p\textbf{\itshape Anova table (one model)}
<<results='asis'>>=
xtable(anova(fm2))
@

\p\textbf{\itshape Anova table (two models)}
<<results='asis'>>=
...
\subsection{More aov}
<<results='asis'>>=
## Venables and Ripley (1997, p. 210)
...
\p\textbf{\itshape Summary table}
<<results='asis'>>=
xtable(summary(npk.aov))
@

<<results='asis'>>=
## Alternative multistratum model
...
\subsection{More lm}
<<results='asis'>>=
## Dobson (1990, p. 9) plant weight data
...
print(temp.table, floating = TRUE)
@
...
print(xtable(lm.D9, caption = "\\tt latex.environments = NULL"),
      floating = TRUE, latex.environments = NULL)
...
\p\textbf{\itshape Left aligned strings with column lines}
<<results='asis'>>=
align(tli.table) <- "|rrl|l|lr|"
tli.table
@

\p\textbf{\itshape Fixed width columns}
<<results='asis'>>=
...
\p\textbf{\itshape Line locations}
...
%
%\p\textbf{\itshape Long tables with the header on each page}
%
