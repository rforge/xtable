...
<<xtablelist>>=
xList <- xtable.xtableList(mtcarsList)
str(xList)
@ %def
...
<<xtablelist1>>=
xList1 <- xtable.xtableList(mtcarsList, digits = c(0,2,0,0,0,1,2))
str(xList1)
@ %def

<<xtablelist2>>=
xList2 <- xtable.xtableList(mtcarsList, digits = c(0,2,0,0,0,1,2),
...
Row and column names can be sanitized.
...
warp.xtblList <- xtable.lsmeans(warp.sum, digits = c(0,0,2,2,0,2,2))
str(warp.xtblList)
@ %def
